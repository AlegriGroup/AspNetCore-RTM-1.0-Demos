...
$ImageName = "username/aspnetcore_hello_world_docker"

# Calculate the name of the container created by the compose file
$ContainerName = "${ProjectName}_aspnetcore_hello_world_docker"
...
function GetUrl () {
    if ([System.String]::IsNullOrWhiteSpace($Machine)) {
        $launchURL = [System.UriBuilder]"http://localhost:28612"
    }
    else {
